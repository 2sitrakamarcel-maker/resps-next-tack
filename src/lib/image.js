...
export function buildSvg(data, date = new Date()) {
  ...
  const leftCard = `
    <g>
      ...
      <text x="282" y="275" font-family="sans-serif" font-size="16" font-weight="800" fill="#0F172A" text-anchor="middle" letter-spacing="2">SURCHARGE DU JOUR</text>
      <text x="282" y="420" font-family="sans-serif" font-size="96" font-weight="900" fill="${isRest ? '#0F172A' : dayPctColor}" text-anchor="middle">${isRest ? '—' : dayPctStr}</text>
      <text x="282" y="475" font-family="sans-serif" font-size="16" font-weight="700" fill="#0F172A" text-anchor="middle">${isRest ? 'Repos' : `${dayStats.filled}/${dayStats.total} exos • ${dayStats.curTotal} reps`}</text>
      <text x="282" y="560" font-family="sans-serif" font-size="12" font-weight="700" fill="#0F172A" text-anchor="middle">${esc(dayName)} • ${esc(date.toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' }))}</text>
    </g>
  `
  const detailContent = (() => {
    if (isRest) {
      return `<text x="812" y="440" font-family="sans-serif" font-size="20" font-weight="800" fill="#0F172A" text-anchor="middle">REPOS</text><text x="812" y="480" font-family="sans-serif" font-size="14" fill="#0F172A" text-anchor="middle">Récup active</text>`
    }
