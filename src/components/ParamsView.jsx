  ...
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 w-full max-w-4xl mx-auto my-2 sm:my-4 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <button onClick={onExport} className="py-3 rounded-xl bg-gray-50 font-bold text-sm hover:bg-white hover:shadow-sm transition-all duration-200 min-h-[44px] cursor-pointer">Export JSON</button>
        <label className="py-3 rounded-xl bg-gray-50 font-bold text-sm hover:bg-white hover:shadow-sm transition-all duration-200 min-h-[44px] flex items-center justify-center cursor-pointer">Import JSON<input type="file" accept=".json" onChange={onImport} className="hidden" /></label>
      </div>
